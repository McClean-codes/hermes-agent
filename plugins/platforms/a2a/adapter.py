...
class A2ARequestHandler(BaseHTTPRequestHandler):
    ...
    def _request_public_url(self) -> str:
        ...
        Priority: profile-scoped A2A_PUBLIC_URL > X-Forwarded-Host / Host
        header (with scheme from X-Forwarded-Proto) > empty. Empty means
        "caller has no info, fall back to bind host". See #41711.

        The URL is captured on the adapter during profile-scoped construction;
        request worker threads do not inherit the profile ContextVar.
        """
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            for tid in self.tasks.fail_orphans(0):
                pass
            _try_persist_task_ledger(self.tasks, _task_ledger_path(), "disconnect/shutdown")
        except Exception:
