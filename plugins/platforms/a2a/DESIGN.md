...
### F. Failure audit ownership and commit-latched side effects (Wave 16 — b8eececc successor, amended)
...
Side effects are commit-latched. Before the authoritative commit point, failures return typed outcomes, make no `agent` append or success `push` audit, emit one owner audit, and leave the required durable state. After commit, `agent` append, success `push` audit, metrics, logs, and callbacks are best-effort and cannot downgrade a committed success; an audit-writer fault after commit is not a durability failure. No post-commit diagnostic may use raw `repr`, `str(exc)`, task/peer objects, or exception text; the loopback and `task_routing` post-commit paths use the adapter-provided `self._bounded_redacted_detail` helper and are enclosed so no exception escapes a published success.

- Remote `_push_out_of_band` commits only after a valid strict `V1_WRAPPED` parse. Before commit, `None` is transport/no-response, non-mapping is invalid_response, top-level error is jsonrpc via `_redacted_jsonrpc_detail`, and strict parser rejection is invalid_response — each returns a typed failure from `_failure_outcome` with one owner audit and no `agent`/`push`. After commit, the `agent` append and success `push` audit are attempted independently; exceptions are caught and logged as warnings with `_bounded_redacted_detail`. A nested reply surfacing via local loopback is a separate child `_push_loopback_in_process` operation; its returned failure already owns its audit and does not downgrade the already committed remote success. Extraction or diagnostic failure after remote commit is bounded-log-only.
...
### G. Recursive hard-final JSON-RPC sanitizer and globally bounded detail (Wave 16 — b8eececc successor, amended)
...
- `_sanitize_jsonrpc_value(value, depth)` with `data` at depth `0`; depths `0..4` representable, depth `5` becomes `"[redacted]"`; `None`/bool/signed-32-bit-int/finite-float retained, out-of-range/non-finite/non-JSON become `"[redacted]"`; mapping visits at most first `16` source entries, keys string-only (non-string becomes `"[redacted]"` without conversion) redacted and capped at `64`, first collision wins; list retains at most first `16` items; tuples/arbitrary iterables become `"[redacted]"`; mapping/list tail truncation adds no marker; depth/type replacement uses `"[redacted]"`; the helper is total and never exposes the raw object.
- `_redacted_jsonrpc_detail(raw_error) -> (error, payload)` allocates a new payload, examines only `code`/`message`/`data` when `raw_error` is a mapping (code integer excluding bool within signed 32-bit, message sanitized string via `_sanitize_string_for_jsonrpc` or `_bounded_redacted_detail` for non-string, data via `_sanitize_jsonrpc_value`), otherwise `{"message": _bounded_redacted_detail(raw_error)}`; ensures `message` fallback to `"[redacted]"` when nothing survives; compact-serializes with `separators (",", ":")`, `ensure_ascii=False`, `allow_nan=False` and enforces UTF-8 `<=2048`; on oversize retains validated code/message and sets `data="[truncated]"` for a re-check; on remaining failure returns constant `{"message":"[redacted]","data":"[truncated]"}`; builds `error` only from retained sanitized code/message with guarded `redact_outbound` and final `_truncate_codepoints(...,300)`; never reuses an unbounded hostile integer; `category` remains `jsonrpc`.

`PushOutcome.error` is built from sanitized `code` and `message` only, capped at `300`, already redacted. Every warning log, failure audit, and `SendResult.error` for JSON-RPC uses only that sanitized error and bounded payload. The sanitizer is private to `adapter.py`; no new public API is added. `task_routing.py` post-commit diagnostics use the adapter-provided bounded helper.
