...
class DynamicReactionMixin:
    ...
    async def _rxn_on_processing_start(self, event: Any) -> bool:
        """Add persona emoji when processing begins and return whether it succeeded."""
        ...
        try:
            if self._reaction_replace_mode:
                added = bool(await self._reaction_set(msg_ref, translated))
            else:
                added = bool(await self._reaction_add(msg_ref, translated))
