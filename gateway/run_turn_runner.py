...
def _canonicalize_query_whitespace(text: str, normalized_set) -> str:
    # Whitespace-tolerant canonicalization for query keys/separators.
    # Strips optional space/tab/newline around the delimiter, key, and '=' for
    # sensitive keys before value matching, so "?  token =\tvalue" and
    # "?token\n=\nvalue" are treated identically to "?token=value" and do not
    # bypass opaque-value redaction. Value whitespace is also stripped to prevent
    # truncation-before-redaction leaks. Non-sensitive keys are left untouched.
    ...
    for m in pattern.finditer(text):
        ...
        if not _is_sensitive_key(key, normalized_set):
            continue
        val_start = m.end()
        # Skip leading whitespace after '=' so "token= opaque" is treated as "token=opaque"
        # Otherwise _find_sensitive_value_end sees leading space and returns immediately.
        while val_start < len(text) and text[val_start] in " \t\n\r":
            val_start += 1
        val_end = _find_sensitive_value_end(text, val_start)
        segments.append((m.start(), m.end(), val_start, val_end, key, m.group(1)))
    if not segments:
        return text
    segments.sort(key=lambda x: x[0])
    out_parts = []
    last = 0
    for prefix_start, prefix_end, val_start, val_end, key, delimiter in segments:
        if prefix_start < last:
            continue
        if val_start < last:
            val_start = last
            if val_start >= val_end:
                continue
        out_parts.append(text[last:prefix_start])
        out_parts.append(f"{delimiter}{key}=")
        segment = text[val_start:val_end]
        no_ws = re.sub(r"[ \t\n\r]+", "", segment)
...
def _strict_incomplete_userinfo_fixup(text: str) -> str:
    """Mask a credential-shaped authority clipped before its terminating ``@``.

    Kanban titles and approval previews can clip a complete ``user:secret@host``
    value before the final sanitizer sees it. A non-numeric authority suffix after
    a user-like component is not a valid host port, so fail closed while retaining
    ordinary ``host:8080`` URLs and IPv6 authorities unchanged.
    """
    import re

    pat = re.compile(r"((?:[A-Za-z][A-Za-z0-9+.-]*:)?//)([^/\s?#]+)")
    known_prefixes = (
        "sk-",
        "ghp_",
        "gho_",
    ...
    def repl(m):
        ...
        suffix_folded = suffix.casefold()
        credential_shaped = len(suffix) >= 8 or any(
            suffix_folded.startswith(prefix.casefold()) for prefix in known_prefixes
        )
        if not credential_shaped:
            return m.group(0)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                fixed = _strict_incomplete_userinfo_fixup(fixed)
                return fixed
            except Exception:
                return "[REDACTED]"
        except Exception:
        ...
        try:
            ...
            try:
                ...
                strict = _strict_url_userinfo_fixup(strict)
                strict = _strict_incomplete_userinfo_fixup(strict)
                try:
...
class TurnRunner:
    ...
    def _reaction_hook_context(self) -> ReactionHookContext:
        ...
        ctx = self._ctx
        source = ctx.source
        message_token = getattr(ctx, "inbound_message_id", None) or getattr(
        ...
        def _is_current() -> bool:
            try:
                ...
                return bool(
                    not self._agent_interrupted()
                    and (not callable(still_current) or still_current())
                )
            except Exception:
                return False
    ...
    def _progress_build_message(
        ...
        try:
            ...
            from agent.display import build_tool_preview, ToolPreview, _display_url

            def _http_url(candidate: str):  # type: ignore[no-redef]
                import re

                m = re.search(r"https?://\S+", str(candidate or ""))
