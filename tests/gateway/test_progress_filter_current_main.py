...
class TestImportantOutputDelivery:
    ...
    async def test_tool_completed_does_not_block_final_reply(self):
        ...
        # Production gateway final delivery via the real adapter processing consumer. Only the
        # adapter's external send effect is controlled by an in-memory ledger.
        from gateway.run import GatewayRunner

        class LedgerAdapter(BasePlatformAdapter):
            def __init__(self):
                super().__init__(PlatformConfig(typing_indicator=False), Platform.TELEGRAM)
                self.sent: list[dict] = []
            ...
            async def send(self, chat_id, content, reply_to=None, metadata=None):
                ...
                return SendResult(success=True, message_id=f"final-{len(self.sent)}")

        gateway = object.__new__(GatewayRunner)
        gateway.config = GatewayConfig()
        adapter = LedgerAdapter()
        adapter.gateway_runner = gateway  # type: ignore[assignment]
        ...
        agent_result = {
            ...
            "tools": [], "failed": False, "completed": True, "api_calls": 1,
            "history_offset": 0, "last_prompt_tokens": 0, "session_id": "sess-final-1",
        }
        session_entry = SessionEntry(
            session_key="agent:main:telegram:group:-1001:12345", session_id="sess-final-1",
            created_at=datetime.now(), updated_at=datetime.now(), platform=Platform.TELEGRAM, chat_type="group",
        )
        event = MessageEvent(
            text="hi",
        ...
        async def gateway_handler(inbound_event):
            # Route the completion through the same callback object as the production turn wiring.
            runner.progress_callback("tool.completed", "terminal", "", {})
            return await gateway._hmwa_deliver_turn_response(
                inbound_event, inbound_event.source, session_entry, session_key, 1,
                agent_result, agent_messages, sanitized, "", False,
            )
        ...
        assert len(adapter.sent) == 1, "tool completion must not duplicate the final reply"
...
class TestProgressRedactionBoundary:
    ...
    def _assert_redacted(self, raw_marker: str, payload: str):
        ...
        # Redacted payload remains non-empty and differs from the raw marker.
