...
def test_push_round_trip_surfaces_peer_reply(monkeypatch, tmp_path):
    ...
    try:
        ...
        monkeypatch.setattr(
            ...
            lambda cid, peer, text: looped.append((cid, peer, text)),
        )
        pushed: list[dict] = []
...
def test_post_write_eof_is_not_a_rescue(monkeypatch):
    ...
    try:
        ...
        # The client is EOF-dead after the write — indistinguishable from a
        # clean close-after-read, so no rescue may fire.
        handler._a2a_client_alive = lambda: probes.append(False) or False  # type: ignore[attr-defined]
        ...
        assert probes == []  # the post-write probe no longer runs at all
    finally:
        adapter._unregister_adapter()
...
def test_write_failure_pushes_and_skips_reprobe(monkeypatch):
    ...
    try:
        ...
        assert len(pushed) == 1
        assert probes == []  # re-probe never ran — the write already failed
    finally:
        adapter._unregister_adapter()
